...
void showUsage( char** );
void cornerHarris( cv::Mat&, std::vector<cv::KeyPoint>&, cv::Mat& );
void imageGradient( cv::Mat&, cv::Mat&, cv::Mat& );
...
//  }
  ...
  //src1 = cv::imread( "/home/breno/VC/hotel-00.png", cv::IMREAD_COLOR );
  ...
  //src2 = cv::imread( "/home/breno/VC/hotel-01.png", cv::IMREAD_COLOR );
  ...
  imageGradient( src_gray1, mag1, angle1 );
  imageGradient( src_gray2, mag2, angle2 );
  
  std::cout<< "Passou aqui!" <<std::endl;
  
  cv::imwrite("gradiente1.jpg", mag1);
  cv::imwrite("gradiente2.jpg", mag2);

  cornerHarris( src_gray1, keypoints_1, angle1 );
  cornerHarris( src_gray2, keypoints_2, angle2 );
...
//  }
  ...
  std::cout<< "Número de Good_matches: " << good_matches.size() <<std::endl;
    cv::Mat fun = cv::getAffineTransform(match2, match1);
...
//    }
  ...
  cv::Mat warp_dst = cv::Mat::zeros( src_gray2.rows, (src_gray1.cols+src_gray2.cols), src_gray1.type() );
  ...
  for(int i = 0; i < warp_dst.rows; i++)
  {
      ...
      {
           if(i < src_gray1.rows and j < src_gray1.cols)
          {
               warp_dst.at<uchar>(i,j) = src_gray1.at<uchar>(i,j);
          }
      }
  }
  
  cv::imwrite("panorama.jpg", warp_dst);
...
void imageGradient(cv::Mat& img, cv::Mat& mag, cv::Mat& angle)
{
    //Calculo do gradiente na direção x e na direção y
    cv::Mat gx, gy; 
...
void cornerHarris( cv::Mat& src, std::vector<cv::KeyPoint>& keypoints, cv::Mat& angles )
{
  int thresh = 180;
  cv::Mat dst, dst_norm;
     ...
     { for( int i = 0; i < dst_norm.cols; i++ )
          {
            if( (int) dst_norm.at<float>(j,i) > thresh )
              {
                float angle = angles.at<float>(j,i);
                std::cout << "Angulo " << angle << std::endl;
                cv::Point2f pt(i, j);
               keypoints.push_back(cv::KeyPoint(pt,40.0,angle,0,0,-1));
